...
const APP_JS: string = [
  ...
  "    var rows=''; rows += drow('id', e.id); rows += drow('outcome', e.outcome + (e.kept?' (kept)':'')); rows += drow('node', (e.node_kind||'\\u2014')+' \\u00b7 depth '+(e.depth||0)); rows += drow('parent', e.parent_id||'root'); rows += drow('claim', e.claim_id||'\\u2014'); rows += drow('value', num(e.value)+(e.baseline!=null?(' (baseline '+num(e.baseline)+')'):'')); if(e.lever||e.altitude) rows += drow('bucket', (e.lever||'?')+' \\u00b7 '+(e.altitude||'?')); if(e.commit_url){ rows += drowRaw('commit', \"<a href='\"+esc(e.commit_url)+\"' target='_blank' rel='noopener'>\"+esc(e.commit||'view commit')+\" \\u2197</a>\"); } else if(e.commit){ rows += drow('commit', e.commit); } if(e.branch) rows += drow('branch', e.branch);",
  ...
  "  function drowRaw(k,htmlv){ return \"<tr><td class='dim' style='width:78px'>\"+k+\"</td><td class='mono'>\"+htmlv+\"</td></tr>\"; }",
  "",
  "  // ---- coverage heatmap ----",
