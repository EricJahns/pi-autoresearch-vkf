...
- **Browser dashboards** — press **Alt+O** (or run `/research-open`). `export_dashboard`
  ...
  - `progress.html` — an **interactive** dashboard (vanilla JS, no build, no
    dependencies): a multi-metric chart (toggle series, log scale, hover tooltips),
    the **search-tree** view with a clickable node detail panel, a lever × altitude
    **coverage heatmap**, belief bars, a filter/sortable experiment table, and a
    light/dark toggle. It reads a `data.json` sidecar and re-renders *in place*, so
    an open tab tracks the run live without losing your filters or scroll.
